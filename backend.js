...
// For Cloudflare Pages, use environment-based storage or fallback to local
// Check if we're on Cloudflare Pages and if persistent storage is available
const IS_CLOUDFLARE = process.env.CF_PAGES === '1';
const HAS_PERSISTENT_DISK = process.env.CF_PAGES_PERSISTENT_DISK === 'true';

// Use persistent paths only if persistent disk is explicitly enabled
const PERSISTENT_DATA_FILE = (IS_CLOUDFLARE && HAS_PERSISTENT_DISK) ? "/tmp/data.json" : DATA_FILE;
const PERSISTENT_UPLOAD_DIR = (IS_CLOUDFLARE && HAS_PERSISTENT_DISK) ? "/tmp/uploads" : UPLOAD_DIR;
const PERSISTENT_BACKUP_DIR = (IS_CLOUDFLARE && HAS_PERSISTENT_DISK) ? "/tmp/backups" : BACKUP_DIR;

// MongoDB Configuration for Cloudflare Pages (Serverless Environment)
const MONGODB_URI = process.env.MONGODB_URI || "mongodb+srv://abhishekmarothiya072002_db_user:vdHyqmX1RLzyccRu@cluster0.xgxmkt2.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0";
...
async function initMongoDB() {
  ...
  try {
    ...
    // Use optimized configuration for Cloudflare Pages (serverless environment)
    client = new MongoClient(MONGODB_URI, {
      maxPoolSize: IS_CLOUDFLARE ? 1 : 10, // Single connection for serverless
      serverSelectionTimeoutMS: 5000, // Keep trying to send operations for 5 seconds
      socketTimeoutMS: 30000, // Shorter timeout for serverless (30 seconds)
      maxIdleTimeMS: 10000, // Close connections quickly in serverless (10 seconds)
    ...
    // Initialize GridFS bucket for media storage (optimized for Cloudflare Pages)
    gridFSBucket = new GridFSBucket(db, { 
      bucketName: 'media',
      chunkSizeBytes: 1024 * 1024, // 1MB chunks for better serverless performance
    ...
    // Create indexes for better performance (optimized for Cloudflare Pages)
    await collection.createIndex({ "type": 1 }, { background: true });
    await collection.createIndex({ "lastUpdated": 1 }, { background: true });
    console.log("📊 Database indexes created for Cloudflare Pages");
...
function isManualDeployment() {
  ...
  const deployId = process.env.CF_PAGES_DEPLOY_ID;
  const gitCommit = process.env.CF_PAGES_COMMIT_SHA;
  const cloudflarePages = process.env.CF_PAGES;
  ...
  // (Cloudflare Pages sets these on manual deployments)
  if (cloudflarePages && (process.env.CF_PAGES_DEPLOY_HOOK || process.env.CF_PAGES_GIT_COMMIT)) {
    console.log(`🔍 Manual deployment detected via Cloudflare Pages environment`);
    return true;
  }
...
async function initializeApp() {
  ...
  console.log(`🌍 Platform: ${IS_CLOUDFLARE ? 'Cloudflare Pages (Serverless)' : 'Local Development'}`);
  console.log(`💾 Storage: ${mongoConnected ? 'MongoDB Atlas + GridFS (Optimized for Serverless)' : 'Local File System'}`);
  console.log(`🔗 MongoDB: ${mongoConnected ? 'Connected with serverless optimizations' : 'Not connected'}`);
